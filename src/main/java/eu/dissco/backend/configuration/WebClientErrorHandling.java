...
public class WebClientErrorHandling {
	...
	public static Mono<ClientResponse> exchangeFilterResponseProcessor(ClientResponse response, String serviceName) {
		...
		if (status.is4xxClientError() || status.is5xxServerError()) {
			...
			return response.bodyToMono(JsonNode.class).flatMap(body -> {
				log.error("An error has occurred with the {} service. Status: {}, response: {}", serviceName, status,
						body);
				return Mono.error(
						new WebProcessingFailedException("An error has occurred with the " + serviceName + " service"));
			});
		}
