...
public class MasJobRecordRepository {
	...
	public List<MasJobRecordFull> getMasJobRecordsByTargetId(String targetId, JobState state, int pageNum,
			int pageSize) {
	...
	public List<MasJobRecordFull> getMasJobRecordsByCreatorId(String creatorId, JobState state, int pageNum,
			int pageSize) {
		var offset = getOffset(pageNum, pageSize);
	...
	private MasJobRecordFull recordToMasJobRecord(Record dbRecord) {
		try {
			var dataNode = dbRecord.get(MAS_JOB_RECORD.ANNOTATIONS) != null
					? mapper.readValue(dbRecord.get(MAS_JOB_RECORD.ANNOTATIONS).data(), JsonNode.class) : null;
			...
			return new MasJobRecordFull(dbRecord.get(MAS_JOB_RECORD.JOB_STATE), dbRecord.get(MAS_JOB_RECORD.MAS_ID),
					dbRecord.get(MAS_JOB_RECORD.TARGET_ID), dbRecord.get(MAS_JOB_RECORD.TARGET_TYPE),
					dbRecord.get(MAS_JOB_RECORD.CREATOR), dbRecord.get(MAS_JOB_RECORD.JOB_ID),
					dbRecord.get(MAS_JOB_RECORD.TIME_STARTED), dbRecord.get(MAS_JOB_RECORD.TIME_COMPLETED), dataNode,
					dbRecord.get(MAS_JOB_RECORD.BATCHING_REQUESTED), Math.toIntExact(ttl),
