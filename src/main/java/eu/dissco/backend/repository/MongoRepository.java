...
public class MongoRepository {
	...
	public JsonNode getByVersion(String id, int version, MongoCollection mongoCollection) throws NotFoundException {
	...
	public List<Integer> getVersions(String id, MongoCollection mongoCollection) throws NotFoundException {
		...
		if (result.first() == null) {
			throw new NotFoundException(
					"Could not find " + id + " in collection: " + mongoCollection.getCollectionName());
		}
		var versions = new ArrayList<Integer>();
		result.forEach(document -> versions.add(document
			.getEmbedded(List.of(PROV_ENTITY, PROV_VALUE, mongoCollection.getVersionProperty()), Integer.class)));
