...
public class DigitalMediaService {
	...
	public JsonApiListResponseWrapper getDigitalMediaObjects(int pageNumber, int pageSize, String path) {
		var mediaPlusOne = repository.getDigitalMediaObjects(pageNumber, pageSize);
		var dataNodePlusOne = mediaPlusOne.stream()
			.map(media -> new JsonApiData(media.getDctermsIdentifier(), FdoType.DIGITAL_MEDIA.getName(),
	...
	public JsonApiWrapper getDigitalMediaById(String id, String path) throws NotFoundException {
		...
		if (mediaObject != null) {
			var dataNode = new JsonApiData(mediaObject.getDctermsIdentifier(), FdoType.DIGITAL_MEDIA.getName(),
					mediaObject, mapper);
	...
	public JsonApiWrapper getDigitalMediaObjectByVersion(String id, int version, String path) throws NotFoundException {
		...
		var dataNode = new JsonApiData(digitalMedia.getDctermsIdentifier(), FdoType.DIGITAL_MEDIA.getName(),
				digitalMedia, mapper);
		return new JsonApiWrapper(dataNode, new JsonApiLinks(path));
	}

	public JsonApiListResponseWrapper getMasJobRecordsForMedia(String targetId, String path, JobState state,
	...
	private static List<String> getMediaIdsFromSpecimen(DigitalSpecimen specimen) {
		return specimen.getOdsHasEntityRelationships()
			.stream()
	...
	// Response Wrapper
	private JsonApiListResponseWrapper wrapResponse(List<JsonApiData> dataNodePlusOne, int pageNumber, int pageSize,
			String path) {
	...
	public JsonApiWrapper getOriginalDataForMedia(String targetId, String path) throws NotFoundException {
		var originalData = repository.getMediaOriginalData(targetId);
		if (originalData != null) {
			return new JsonApiWrapper(new JsonApiData(targetId, FdoType.DIGITAL_MEDIA.getName(), originalData),
	...
	private JsonNode flattenAttributes(DigitalMedia digitalMedia, DigitalSpecimen digitalSpecimen) {
	...
	public JsonApiListResponseWrapper scheduleMass(String id, List<MasJobRequest> masRequests, String path,
	...
	public byte[] getMediaDerivative(String suffix) throws NotFoundException, ProcessingFailedException {
	...
	public byte[] getMediaThumbnail(String suffix) throws NotFoundException, ProcessingFailedException {
