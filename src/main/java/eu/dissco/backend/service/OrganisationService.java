...
public class OrganisationService {
  ...
  public JsonApiListResponseWrapper getOrganisations(String path, int pageNumber, int pageSize) {
    ...
    organisations.forEach(
        org -> dataNode.add(new JsonApiData(org.ror(), "organisation", org, mapper)));
  ...
  public JsonApiListResponseWrapper getCountries(String path, int pageNumber, int pageSize) {
    ...
    List<JsonApiData> dataNode = new ArrayList<>();
    countries.forEach(country -> dataNode.add(
        new JsonApiData(country.countryCode(), "country", country, mapper)));
