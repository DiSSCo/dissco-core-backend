...
public class DigitalSpecimenService {
	...
	private static Set<MappingTerm> retrieveTaxRanks(Map<TaxonMappingTerms, List<String>> mappedParams) {
	...
	public JsonApiListResponseWrapper getSpecimen(int pageNumber, int pageSize, String path) throws IOException {
		...
		var dataNodePlusOne = digitalSpecimenList.stream()
			.map(specimen -> new JsonApiData(specimen.getDctermsIdentifier(), FdoType.DIGITAL_SPECIMEN.getName(),
					specimen, mapper))
			.toList();
		return wrapListResponse(dataNodePlusOne, elasticSearchResults.getLeft(), pageSize, pageNumber, path);
	}
	...
	public JsonApiWrapper getSpecimenById(String id, String path) throws NotFoundException {
		...
		if (digitalSpecimen != null) {
			var dataNode = new JsonApiData(digitalSpecimen.getDctermsIdentifier(), FdoType.DIGITAL_SPECIMEN.getName(),
					digitalSpecimen, mapper);
	...
	public JsonApiWrapper getSpecimenByVersionFull(String id, int version, String path) throws NotFoundException {
	...
	public JsonApiListResponseWrapper getMasJobRecordsForSpecimen(String targetId, JobState state, String path,
			int pageNum, int pageSize) {
		return masJobRecordService.getMasJobRecordByTargetId(targetId, state, path, pageNum, pageSize);
	}

	public JsonApiWrapper getOriginalDataForSpecimen(String targetId, String path) throws NotFoundException {
		var originalData = repository.getSpecimenOriginalData(targetId);
		if (originalData != null) {
			return new JsonApiWrapper(new JsonApiData(targetId, FdoType.DIGITAL_SPECIMEN.getName(), originalData),
					new JsonApiLinks(path));
		}
		log.warn("Unable to find specimen {}, can not retrieve original data", targetId);
		throw new NotFoundException("Can not find specimen " + targetId + ", can not retrieve origina; data");
	}
	...
	private JsonApiWrapper mapFullSpecimen(String id, String path, DigitalSpecimen specimen) {
		...
		var attributeNode = mapper.valueToTree(new DigitalSpecimenFull(specimen, digitalMedia, annotation));
		return new JsonApiWrapper(
				new JsonApiData(specimen.getDctermsIdentifier(), FdoType.DIGITAL_SPECIMEN.getName(), attributeNode),
				new JsonApiLinks(path));
	}

	public JsonApiWrapper getSpecimenByVersion(String id, int version, String path) throws NotFoundException {
		var specimenNode = mongoRepository.getByVersion(id, version, MongoCollection.DIGITAL_SPECIMEN);
		var specimen = mapResultToSpecimen(specimenNode);
		var dataNode = new JsonApiData(specimen.getDctermsIdentifier(), FdoType.DIGITAL_SPECIMEN.getName(), specimen,
				mapper);
		return new JsonApiWrapper(dataNode, new JsonApiLinks(path));
	}
	...
	public JsonApiListResponseWrapper getDigitalMedia(String id, String path) throws NotFoundException {
		...
		var dataNodes = digitalMedias.stream()
			.map(media -> new JsonApiData(media.getId(), FdoType.DIGITAL_MEDIA.getName(), media, mapper))
			.toList();
	...
	private JsonApiListResponseWrapper wrapListResponseSearchResults(
			Pair<Long, List<DigitalSpecimen>> digitalSpecimenSearchResult, int pageNumber, int pageSize, String path) {
		var dataNodePlusOne = digitalSpecimenSearchResult.getRight()
			.stream()
			.map(specimen -> new JsonApiData(specimen.getDctermsIdentifier(), FdoType.DIGITAL_SPECIMEN.getName(),
					specimen, mapper))
			.toList();
	...
	private int getIntParam(String paramName, MultiValueMap<String, String> params, String defaultValue) {
	...
	private <T extends MappingTerm> Map<T, List<String>> mapParamsKeyword(MultiValueMap<String, String> requestParams,
			Map<String, T> acceptedParams) throws UnknownParameterException {
	...
	private <T extends MappingTerm> void validateExistsQuery(T mappedParam, Map.Entry<String, List<String>> entry)
			throws UnknownParameterException {
		if (mappedParam instanceof MissingMappingTerms && !entry.getValue().isEmpty()) {
			if (entry.getValue().size() > 1) {
				throw new UnknownParameterException("Parameter :" + entry.getKey() + " accepts at most 1 value");
			}
			if (!entry.getValue().getFirst().equals("true") && !entry.getValue().getFirst().equals("false")) {
				throw new UnknownParameterException("Parameter : " + entry.getKey() + " must be either true or false");
			}
		}
	}

	public JsonApiWrapper taxonAggregations(MultiValueMap<String, String> params, String path)
	...
	public JsonApiWrapper aggregations(MultiValueMap<String, String> params, String path)
			throws IOException, UnknownParameterException {
		var mappedParams = mapParamsKeyword(params, getParamMapping());
		var aggregations = elasticRepository.getAggregations(mappedParams, DefaultMappingTerms.getAggregationSet(),
				false);
		var dataNode = new JsonApiData(String.valueOf(params.hashCode()), AGGREGATIONS_TYPE,
				mapper.valueToTree(aggregations));
		return new JsonApiWrapper(dataNode, new JsonApiLinks(path));
	...
	public JsonApiWrapper discipline(String path) throws IOException {
		...
		return new JsonApiWrapper(dataNode, new JsonApiLinks(path), new JsonApiMeta(disciplineResult.getLeft()));
	}
	...
	public JsonApiWrapper searchTermValue(String name, String value, String path, boolean sort)
		...
		if (mappedTerm != null) {
			var aggregations = elasticRepository.aggregateTermValue(name, mappedTerm.fullName(), value, sort);
	...
	public JsonApiListResponseWrapper scheduleMass(String id, List<MasJobRequest> masRequests, String orcid,
