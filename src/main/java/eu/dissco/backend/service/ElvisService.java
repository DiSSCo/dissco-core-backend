...
public class ElvisService {
	...
	public InventoryNumberSuggestionResponse suggestInventoryNumber(String searchValue, int pageNumber, int pageSize)
		...
		return new InventoryNumberSuggestionResponse(results.getLeft(),
				specimenList.stream()
					.map(specimen -> new InventoryNumberSuggestion(specimen.getOdsPhysicalSpecimenID(),
							removeDoiProxy(specimen.getDctermsIdentifier())))
					.toList());
	}

	private Pair<Long, List<DigitalSpecimen>> searchElastic(String inventoryNumber, int pageNumber, int pageSize)
			throws IOException {
		var map = Map.of(DefaultMappingTerms.PHYSICAL_SPECIMEN_ID, List.of("*" + inventoryNumber + "*"),
	...
	private static ElvisSpecimen buildElvisSpecimen(DigitalSpecimen digitalSpecimen) {
		...
		String scientificName = getTaxonField(taxonIds.stream().map(TaxonIdentification::getDwcScientificName));
		String scientificNameAuthorship = getTaxonField(
				taxonIds.stream().map(TaxonIdentification::getDwcScientificNameAuthorship));
		String specificEpithet = getTaxonField(taxonIds.stream().map(TaxonIdentification::getDwcSpecificEpithet));
		...
		String vernacularName = getTaxonField(taxonIds.stream().map(TaxonIdentification::getDwcVernacularName));

		return new ElvisSpecimen(digitalSpecimen.getDctermsIdentifier(), // inventory
																			// number
				buildElvisTitle(digitalSpecimen), // title
				digitalSpecimen.getDwcCollectionCode() == null ? "" : digitalSpecimen.getDwcCollectionCode(), // collection
																												// code
				digitalSpecimen.getOdsPhysicalSpecimenID(), // catalog number
				digitalSpecimen.getOdsOrganisationCode() == null ? "" : digitalSpecimen.getOdsOrganisationCode(), // institution
																													// code
	...
	private static String getTaxonField(Stream<String> taxonFields) {
		var taxonList = taxonFields.map(a -> a == null ? "" : a).toList();
	...
	private static String buildElvisTitle(DigitalSpecimen digitalSpecimen) {
		...
		title.append(digitalSpecimen.getOdsPhysicalSpecimenID())
			.append(", ")
			.append(digitalSpecimen.getOdsOrganisationName());
		return title.toString();
	}

	private static List<TaxonIdentification> getBestTaxonIdentification(DigitalSpecimen digitalSpecimen) {
		if (digitalSpecimen.getOdsHasIdentifications().isEmpty()
				|| digitalSpecimen.getOdsHasIdentifications().getFirst().getOdsHasTaxonIdentifications().isEmpty()) {
			return List.of();
		}
