	...
	private List<MachineAnnotationService> populateDatabase() {
		...
		for (int i = 0; i < 26; i++) {
			...
			if (i % 2 == 0) {
				...
				masRecord = MachineAnnotationServiceUtils.givenMas(PREFIX + "ABC-123-XY" + i, CREATED);
			}
	...
	private void createRecord(MachineAnnotationService mas) {
		...
		context.insertInto(MACHINE_ANNOTATION_SERVICE)
			...
			.set(MACHINE_ANNOTATION_SERVICE.TOMBSTONED,
					mas.getOdsHasTombstoneMetadata() != null
							? mas.getOdsHasTombstoneMetadata().getOdsTombstoneDate().toInstant() : null)
