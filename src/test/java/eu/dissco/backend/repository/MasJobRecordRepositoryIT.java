...
class MasJobRecordRepositoryIT extends BaseRepositoryIT {
  ...
  void testGetMasJobRecordByIdNotPresent() throws JsonProcessingException {
    ...
    // When
    var result = masJobRecordRepository.getMasJobRecordById(JOB_HANDLE);
  ...
  void testCreateNewMasJobRecord() {
    // Given
    var mjr = new MasJobRecord(JOB_HANDLE, AnnotationState.SCHEDULED, ID, ID_ALT, ORCID);
  ...
  void testMarkMasJobRecordsAsFailed() throws Exception {
    ...
    // When
    masJobRecordRepository.markMasJobRecordsAsFailed(List.of(JOB_HANDLE));
    var result = context.select(MAS_JOB_RECORD.JOB_ID, MAS_JOB_RECORD.STATE,
            MAS_JOB_RECORD.TIME_COMPLETED)
        .from(MAS_JOB_RECORD)
        .where(MAS_JOB_RECORD.JOB_ID.eq(JOB_HANDLE.toString()))
        .fetchSingle();
  ...
  void testMarkMasJobRecordAsRunning() throws Exception {
    ...
    // When
    masJobRecordRepository.markMasJobRecordAsRunning(ID_ALT, JOB_HANDLE);
    var result = context.select(MAS_JOB_RECORD.JOB_ID, MAS_JOB_RECORD.STATE)
        .from(MAS_JOB_RECORD)
        .where(MAS_JOB_RECORD.JOB_ID.eq(JOB_HANDLE))
        .fetchSingle();
    var state = result.get(MAS_JOB_RECORD.STATE);
