...
class MasJobRecordRepositoryIT extends BaseRepositoryIT {
  ...
  void testCreateNewMasJobRecord() {
    // Given
    var mjr = new MasJobRecord(JOB_ID, JobStates.SCHEDULED, ID, ID_ALT,
        TargetTypes.DIGITAL_SPECIMEN, ORCID);
    ...
    assertThat(result.get(MAS_JOB_RECORD_NEW.JOB_STATE)).isEqualTo(JobStates.SCHEDULED);
  }
  ...
  void testMarkMasJobRecordsAsFailed() throws Exception {
    ...
    assertThat(state).isEqualTo(JobStates.FAILED);
  }
  ...
  void testMarkMasJobRecordAsRunning() throws Exception {
    ...
    // Then
    assertThat(state).isEqualTo(JobStates.RUNNING);
  }
