...
public class MachineAnnotationServiceUtils {
  ...
  public static JsonApiListResponseWrapper givenScheduledMasResponse(
      MasJobRecord masJobRecord,
      String path) {
    var links = new JsonApiLinksFull(path);
    var masRecords = List.of(
        new JsonApiData(masJobRecord.jobId(), "MachineAnnotationServiceJobRecord", masJobRecord, MAPPER));
    return new JsonApiListResponseWrapper(masRecords, links, new JsonApiMeta(masRecords.size()));
  }
