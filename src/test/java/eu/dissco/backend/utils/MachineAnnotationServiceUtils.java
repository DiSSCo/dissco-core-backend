...
public class MachineAnnotationServiceUtils {
	...
	public static JsonApiListResponseWrapper givenMasResponse(String path) {
		...
		var masRecords = List.of(new JsonApiData(givenMas().getId(), "MachineAnnotationService", givenMas(), MAPPER));
		return new JsonApiListResponseWrapper(masRecords, links, new JsonApiMeta(masRecords.size()));
	}

	public static JsonApiListResponseWrapper givenMasResponse(MachineAnnotationService masRecord, String path) {
		var links = new JsonApiLinksFull(path);
		var masRecords = List.of(new JsonApiData(masRecord.getId(), "ods:MachineAnnotationService", masRecord, MAPPER));
		return new JsonApiListResponseWrapper(masRecords, links, new JsonApiMeta(masRecords.size()));
	}

	public static MasSchedulingRequest givenMasRequest() {
		return new MasSchedulingRequest(
				new MasSchedulingData("MasRequest", new MasSchedulingAttributes(List.of(givenMasJobRequest()))));
	}
	...
	public static MasJobRequest givenMasJobRequest(boolean batching) {
		return new MasJobRequest(MAS_ID, batching);
	}
	...
	public static MachineAnnotationService givenMas(String id, Instant deleted) {
		return givenMas(id, deleted, new OdsHasTargetDigitalObjectFilter(), false, TTL_DEFAULT);
	}

	public static MachineAnnotationService givenMas(OdsHasTargetDigitalObjectFilter filters, boolean batching) {
		return givenMas(MAS_ID, null, filters, batching, TTL_DEFAULT);
	}
	...
	public static MachineAnnotationService givenMas(String id, Instant deleted, OdsHasTargetDigitalObjectFilter filters,
			boolean batching, int ttl) {
		var mas = new MachineAnnotationService().withId(id)
		...
		if (deleted != null) {
			...
			mas.setOdsHasTombstoneMetadata(new TombstoneMetadata().withOdsTombstoneDate(Date.from(deleted)));
		}
		return mas;
	}
